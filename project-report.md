...
During this process, there were a few issues that may have caused inaccuracies in our results. The recorded times are inaccurate of the users' time of each completed subtask. This is because some subtasks occurred too quickly which caused some delay in the timekeeper's reaction time. This happened frequently in the last task which was question creation. In the question creation task, each time was recorded from the first subtask to the end of the last subtask. An average time for each subtask was calculated by dividing the time taken for the total task divided by the nine subtasks. Time was also counted when instructions were given between tasks and subtasks. This does not represent the true time taken as users would pause and take time to interpret the question before actually moving the mouse and interacting with the prototype.  
